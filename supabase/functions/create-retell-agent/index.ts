...
function buildOutboundFlowNodes(config: AgentConfig) {
  ...
  const transferNote = forwarding_number
    ? `If the lead is highly interested and ready to speak with an advisor now, offer to transfer them. The transfer number is ${forwarding_number}.`
    : "";
  ...
  return [
    ...
    },
    {
      id: "not_interested",
      type: "conversation",
      instruction: { type: "prompt", text: `The lead is not interested. Be gracious, professional, and wish them well. Let them know they can reach out to ${company_name} in the future if their situation changes.` },
      extract_dynamic_variable: [
